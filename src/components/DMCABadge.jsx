...
const env = (key) => (import.meta?.env ? import.meta.env[key] : undefined);
...
const DMCABadge = ({
  ...
  const isConfigured = !!env('VITE_DMCA_BADGE_URL') && !!env('VITE_DMCA_STATUS_URL');
  ...
  return (
    ...
    >
      <div className={`dmca-badge group relative ${themeClasses[theme] ?? ''} transition-all duration-200 hover:shadow-md`}>
        <a
...
DMCABadge.propTypes = {
  ...
  position: PropTypes.oneOf(['inline', 'fixed-bottom-right', 'fixed-bottom-left', 'center', 'float-right']),
  theme: PropTypes.oneOf(['light', 'dark', 'transparent', 'minimal']),
